 ...
 * 
 */
class HelloCashPaymentGateway implements PaymentGatewayContract
{
	...
	public function withdraw()
	{
		$phone = $this->phone;
		//dd($phone);
$bankbalance = DB::connection('mysql2')->select('SELECT * FROM hellocash WHERE phone=?',[$phone]);
...
else
{
	foreach ($bankbalance as $row) {$balancebank = $row->balance;}
		...
		if ($balancebank > $this->amount) {
			//UPDATE `hellocash` SET `id`=[value-1],`phone`=[value-2],`balance`=[value-3] WHERE 1
			...
			$adminhelloamount = DB::connection('mysql2')->select('SELECT * FROM hellocash WHERE phone=?',[$adminphone->phone]);
			...
			if ($userbank->save() && $trans->save()) {  
				...
				->select('UPDATE hellocash SET balance=? WHERE phone=?',[$amountleft,$phone]);
				
				$adminbalance = DB::connection('mysql2')
				->select('UPDATE hellocash SET balance=? WHERE phone=?',[$admintotalamount,$adminphone->phone]);
				return 1;
			}
			else
			{
