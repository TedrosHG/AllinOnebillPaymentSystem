 ...
 * 
 */
class CbePaymentGateway implements PaymentGatewayContract
{
	private $user;
	private $amount;
	private $phone;
...
public function withdraw()
	{
		$phone = $this->phone; 
	$bankbalance = DB::connection('mysql2')->select('SELECT * FROM cbe WHERE phone=?',[$phone]); 
	...
	foreach ($bankbalance as $row) {$balancebank = $row->balance;} 
		if ($balancebank > $this->amount) {
			//UPDATE `cbe` SET `id`=[value-1],`phone`=[value-2],`balance`=[value-3] WHERE 1
			...
			$adminhelloamount = DB::connection('mysql2')->select('SELECT * FROM cbe WHERE phone=?',[$adminphone->phone]);
			...
			if ($userbank->save() && $trans->save()) {  
				...
				->select('UPDATE cbe SET balance=? WHERE phone=?',[$amountleft,$phone]);
				
				$adminbalance = DB::connection('mysql2')
				->select('UPDATE cbe SET balance=? WHERE phone=?',[$admintotalamount,$adminphone->phone]);
				return 1;
			}
			else
			{
	...
	public function deposite()
	{
		...
		$phone = $this->phone;
		$bankbalance = DB::connection('mysql2')
		->select('SELECT * FROM cbe WHERE phone=?',[$phone]);
if (count($bankbalance)==0) 
{
  	return 4;	
}
